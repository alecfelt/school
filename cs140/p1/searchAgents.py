...
class CornersProblem(search.SearchProblem):
  ...
  def __init__(self, startingGameState, costFn = lambda x: 1, warn=True):
    """
    ...
    self.startState = (self.startingPosition, list(self.goal))
    self.costFn = costFn
    # For display purposes
    self._visited, self._visitedlist, self._expanded = {}, [], 0
    ...
    for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
        ...
        if not self.walls[nextx][nexty]:
            ...
            for goal in self.corners:
                if goal == nextState:
                    goals[self.corners.index(goal)] = True
            cost = self.costFn(nextState)
            succ.append( ( (nextState, goals), action, cost) )
...
def cornersHeuristic(state, problem):
    ...
    for corner in corners:
        dist = mazeDistance(space, corner, problem.gameState)
        if dist > farthest:
            farthest = dist
    heurVal = farthest
    return heurVal
...
def foodHeuristic(state, problem):
  ...
  "*** Your Code Here ***"
  return farthestFood(state, problem)
...
def farthestFood(state, problem):
    ...
    farthest = -1
    for pos in foodList:
        dist = mazeDistance(position, pos, problem.gameState)
        if dist > farthest:
            farthest = dist
    return farthest

def closeFoodLeft(state, problem):
    position = state[0]
    foodList = state[1].asList()
    if not len(foodList):
        return 0
